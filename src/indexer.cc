...
void Indexer::populateMainIndex(unsigned int trigram) {
  mainTrigramTracker->set(trigram);
}
...
void Indexer::indexFile(std::string filePath) {
  ...
  filestream.close();
  perFileTrigramStore->clearAndIterate(
      std::bind(&Indexer::populateMainIndex, this, std::placeholders::_1));
}

void Indexer::indexDir(std::string directory) {
  walkDir(directory,
          std::bind(&Indexer::indexFile, this, std::placeholders::_1));
}
